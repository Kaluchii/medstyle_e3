...
$(document).ready(function() {
    ...
    $(document).on('click', '.from-life', function(event){
        // which was clicked?
        var clicked = $(this).data('img')-1;
    ...
    $(document).on('click', ".sb-item", function(event){
        loose = false;
    });

    //Клик по пустой области - закрытие галереи
    $(document).on('click', ".smoothbox-centering", function(event){
        if(loose)
        {
            $('.smoothbox').fadeOut('slow', function() {
                $('.smoothbox').remove();
