<section class="offers">
    <div class="offers__wrap fade js_animation" data-offset="40">
        ...
        <p class="offers__title title--l1 fade fadeInUp__child">Акции и скидки</p>
        <ul class="offers__list">
            @php $i = 1 @endphp
            @foreach($stocks->stocks_group as $item)
                <li class="offers__item offers__item--services fade fadeInUp__child fade__delay-{{ $i++ }}">
                    <a href="{{$item->link}}" class="offers__item-link"></a>
                    <div class="offers__item-img-wrap offers__item-img-wrap--services">
                        <img src="{{$item->img->link}}?{{$item->img->cache_index}}" alt="{{$item->stock_name}}" class="offers__img">
                    </div>
                    <div class="offers__info-wrap offers__info-wrap--services">
                        <p class="offers__item-title">{{ $item->stock_name }}</p>
